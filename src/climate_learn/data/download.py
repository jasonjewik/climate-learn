...
def _download_copernicus(root, dataset, variable, year, pressure=False, api_key=None):
    """Downloads data from the Copernicus Climate Data Store (CDS). Data is
        stored at `root/dataset/variable/` as NetCDF4 (`.nc`) files. Skips the
        download if a file of the expected naming convention already exists at
        the download destination. More info:
...
def _download_esgf(
    ...
    """Downloads data from the Earth System Grid Federation (ESGF). Data is
        stored at `root/dataset/pre-regrided/variable/` as a NetCDF4 (`.nc`)
        file. Skips the download if a file of the expected naming convention
        already exists at the download destination. More info:
...
def _download_weatherbench(root, dataset, variable, resolution="1.40625"):
    """Downloads data from WeatherBench. Data is stored at
        `root/dataset/resolution/variable/` as NetCDF4 (`.nc`) files. Skips the
        download if a file of the expected naming convention already exists at
        the download destination. More info: https://mediatum.ub.tum.de/1524895
...
def download(source, **kwargs):
    ...
    :param \**kwargs: arguments to pass to the source-specific download
        function: :py:func:`download_copernicus`,
        :py:func:`download_weatherbench`, :py:func:`download_esgf`
    """
